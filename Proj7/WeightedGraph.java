...
public class WeightedGraph<V> {
	...
	public WeightedGraph() {
		// Initializes weightedGraph data structure and observerList
	...
	public void addVertex(V vertex) {
		// If vertex is present, throw exception else add vertex
	...
	public boolean containsVertex(V vertex) {
		// Check if key is present in map, if so then return true else false
		if (weightedGraph.containsKey(vertex) == true) {
			return true;
		} else {
			return false;
		}
	...
	public void addEdge(V from, V to, Integer weight) {
		// Throws exception if either vertex isnt present or weight is negative
		if (this.containsVertex(from) == false || this.containsVertex(to) 
				== false || weight < 0) {
			throw new IllegalArgumentException();
		} else {
			// Else input the weighted edge into the original map
			Map<V, Integer> toWeight = weightedGraph.get(from);
			toWeight.put(to, weight);
			weightedGraph.put(from, toWeight);
		}
	}
	...
	public Integer getWeight(V from, V to) {
		// if either vertex isnt present, throw exception
		if (this.containsVertex(from) == false || 
				this.containsVertex(to) == false) {
			throw new IllegalArgumentException();
		} else {
			// return integer associated with edge, else return null
			Map<V, Integer> toMap = weightedGraph.get(from);
	...
	public void DoBFS(V start, V end) {
		// notify observers start of BFS
		for (GraphAlgorithmObserver<V> observers : observerList) {
			observers.notifyBFSHasBegun();
		}
		// Data structures utilized in BFS
		Set<V> visSet = new HashSet<V>();
		...
		while (q.peek() != null) {
			...
			V vector = q.element();
			q.remove();
			// if visited set doesn't contain target vertex
			if (visSet.contains(vector) != true) {
				visSet.add(vector);
				// notify observers on visit
				for (GraphAlgorithmObserver<V> observers : observerList) {
					observers.notifyVisit(vector);
					if (vector.equals(end)) {
						for (GraphAlgorithmObserver<V> obs : observerList) {
							obs.notifySearchIsOver();
						}
						return;
					}
				}
				/// add adjacent vertexes to queue if not in visited
				for (V vert : weightedGraph.get(vector).keySet()) {
					if (visSet.contains(vert) == false) {
						q.add(vert);
					}
				}
			}
	...
	public void DoDFS(V start, V end) {
		...
		// push start vector onto stack
		stack.push(start);
		// if stack isnt empty
		while (stack.isEmpty() != true) {
			// Pop vector off stack
			V vector = stack.peek();
			stack.pop();
			// if visited doesn't have popped vector
			if (visSet.contains(vector) == false) {
				visSet.add(vector);
				// notify observers
				for (GraphAlgorithmObserver<V> observers : observerList) {
					observers.notifyVisit(vector);
				...
				// add adjacent vectors to stack if not in visited
				for (V vert : weightedGraph.get(vector).keySet()) {
					if (visSet.contains(vert) == false) {
						stack.push(vert);
					}
	...
	public void DoDijsktra(V start, V end) {
		// notify observers
		for (GraphAlgorithmObserver<V> obs : observerList) {
			obs.notifyDijkstraHasBegun();
		}
		// data structures for dijkstra
		Set<V> finSet = new HashSet<V>();
		Map<V, V> Pred = new HashMap<>();
		Map<V, Integer> Cost = new HashMap<>();
		// set initial values for vertexes in maps
		for (V vert : weightedGraph.keySet()) {
			Cost.put(vert, 999);
			Pred.put(vert, null);
		}
		// start cost 0 on initial vertex
		Cost.put(start, 0);
		// if finished set doesn't have all vectors in graph
		while (finSet.containsAll(weightedGraph.keySet()) == false) {
			// locate vertex with smallest cost
			V K = findSmallest(finSet, Cost);
			finSet.add(K);
			// notify observers
			for (GraphAlgorithmObserver<V> obs : observerList) {
				obs.notifyDijkstraVertexFinished(K, Cost.get(K));
			}
			// iterate through all adj vertexes and update costs and
			// predecessors
			for (V vertex : weightedGraph.get(K).keySet()) {
				if (finSet.contains(vertex) == false) {
					if (Cost.get(K) + getWeight(K, vertex) < Cost.get(vertex)) {
						Cost.put(vertex, Cost.get(K) + getWeight(K, vertex));
						Pred.put(vertex, K);
					}
				}
			}
		}
		// create list of fastest path and notify observers
		List<V> lowestCostPath = lowestCost(Pred, start, end);
		for (GraphAlgorithmObserver<V> obs : observerList) {
			obs.notifyDijkstraIsOver(lowestCostPath);
		}
	}

	// locates smallest vertex cost
	public V findSmallest(Set<V> set, Map<V, Integer> map) {
		int smallestCost = 1000;
		V smallestVert = null;
		// runs through list of keys in map
		// if the set doesn't contain the vertex then compare
		for (V vert : map.keySet()) {
			if (set.contains(vert) == false) {
				if (map.get(vert) < smallestCost) {
					// stores vertex with smallest Cost
					smallestCost = map.get(vert);
					smallestVert = vert;
				}
			}
		}
	...
	// returns list of lowest cost path from start to end
	public List<V> lowestCost(Map<V, V> map, V start, V end) {
