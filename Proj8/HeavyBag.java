...
public class HeavyBag<T> extends AbstractCollection<T> implements Serializable {
	 ...
	 * it does return a String and that two different HeavyBags return two different
	 * Strings.
	 */
	@Override
	public String toString() {
		String str = new String();
		Set<T> keySet = data.keySet();
		for (T obj : keySet) {
			for (int i = 0; i < data.get(obj); i++) {
				str += obj;
			}
		}
		return str;
	...
	public boolean equals(Object o) {
		...
		if (o instanceof HeavyBag == false) {
			return false;
		}

		HeavyBag<?> bag = (HeavyBag<?>) o;
		boolean isEquals = false;
		if (this.containsAll(bag.uniqueElements())) {
			for (Object element : bag) {
				if (bag.data.get(element) == this.data.get(element)) {
					isEquals = true;
				} else {
					isEquals = false;
				}
		...
		return this.hashCode() == bag.hashCode() && this.size() == bag.size() && isEquals;
	}

	/**
	 * Return a hashCode that fulfills the requirements for hashCode (such as any
	 * two equal HeavyBags must have the same hashCode) as well as desired
	...
	/**
	 * <P>
	 * Returns an iterator over the elements in a heavy bag. Note that if a Heavy
	 * bag contains 3 a's, then the iterator must iterate over 3 a's individually.
	 * </P>
	 */
	@Override
	public Iterator<T> iterator() {
		return new Iterator<T>() {
			private int curr = 0;
			private T currElement = null;
			private int elementOccurence = 0;
			private Iterator<T> valueIter = data.keySet().iterator();
			...
			public T next() {
				if (hasNext()) {
					...
					if (elementOccurence == 0) {
						currElement = valueIter.next();
						elementOccurence = data.get(currElement);
					}
					curr++;
				} else {
			...
			public void remove() {
				...
				if (data.get(currElement) >= 1) {
					data.put(currElement, data.get(currElement) - 1);
				} else {
	 ...
	 * 
	 * This operation can take time proportional to the number of unique objects in
	 * the Bag, but no more.
	 * 
	...
	public T choose(Random r) {
		...
		for (T currEl : data.keySet()) {
			counter += data.get(currEl);
			if (counter > random) {
				return currEl;
			}
		}
	...
	public boolean remove(Object o) {
		if (data.containsKey(o) == true && data.get(o) > 0) {
			if (data.get(o) > 1) {
				data.put((T) o, data.get(o) - 1);
				size--;
			} else {
				data.remove(o);
				size--;
